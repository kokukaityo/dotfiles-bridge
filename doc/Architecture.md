...
1. **コンフリクトマーカー検出**: `^(<<<<<<<|=======|>>>>>>>)` パターンでワークツリーをスキャンし、未解決のマージコンフリクトが残っていればpushをブロックする。
2. **シークレットスキャン**: push対象のコミット差分（リモートとの差分）に対し、以下の正規表現でシークレットらしきパターンを検出する。
   - 検出パターン: `(api[_-]?key|secret|password|token|BEGIN (RSA|OPENSSH) PRIVATE KEY)\s*[:=]`
   - 除外パターン: `(example|placeholder|<your|\$\{)` — サンプル値や変数参照は誤検知として除外する
   - 環境変数 `SKIP_SECRET_SCAN=1` を設定するとスキャンをスキップできる

#### post-merge

pull（fast-forward merge）後に `dotfiles link` を再実行し、symlinkを最新状態に更新する。
