...
- `bin/dotfiles` 内のコマンド関数は `cmd_<subcommand>` とする
- `lib/` 内の関数は、動詞から始まる snake_case とする（例: `detect_os`, `generate_commit_msg`）
- 変数を引用し、未設定を許容する展開には `${VAR:-}` または `${VAR:-default}` を使う
- ユーザー向けメッセージは日本語にする
- エラーは `[dotfiles]`、`[sync]` など適切なプレフィックスを付け、stderr へ出力する
- macOS で利用できない `readlink -f` は使わない。必要なら `bin/dotfiles` の `resolve_path()` を参照する
- GNU 固有機能への依存を増やす場合は、macOS と Git Bash での代替手段を確認する

### 命名規則

- 識別子は単数形を基本とする（ディレクトリ名・クラス名など、種類や型にラベルを貼るもの）
- 以下は複数形を用いる:
  - iterable（配列など、複数要素を保持する変数）
  - 実務上の慣習として複数形が定着しているもの（REST URI など）
  - 外部ツール・フレームワークが規約として読む名前（`skills/`, `commands/`, `.git/hooks/` 等）
...
1. **Docker E2E**（最優先）: `ubuntu:24.04` でクリーン環境を作り、`dotfiles init` → `setup` → `push` → `pull` を実際に実行。「他人のクリーン環境で動くか」を検証する
...
2. CLI に影響する場合は `bash bin/dotfiles help` など、副作用のない経路で基本動作を確認する
...
- [ ] `lib/hooks/` 内のスクリプトに変更が必要か
- [ ] `template/` への変更が必要か（データリポジトリ側に影響する変更の場合）
- [ ] エラーメッセージに適切なプレフィックス（`[dotfiles]`, `[sync]` 等）があるか
...
| # | 深刻度 | 内容 | 該当箇所 |
|---|---|---|---|
| 1 | 中 | `pre-push` はワークツリーのみ検査。一度コミットして次のコミットで削除したシークレットは、push 対象の履歴に残っていても検出されない | `lib/hooks/pre-push` |
| 2 | 低 | `post-merge` で `link.sh` が失敗しても `"symlink refreshed"` と表示される | `lib/hooks/post-merge` |
...
| `.agents/note/` | 作業ログ・備忘録。調査結果や判断の記録 |
