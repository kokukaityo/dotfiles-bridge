...
```powershell
Copy-Item -Recurse ~\.claude ~\.claude.bak
Copy-Item -Recurse ~\.codex ~\.codex.bak
Copy-Item -Recurse ~\.agents ~\.agents.bak
```
...
$env:PATH = "d:\work\dotfile\work\dist;$env:PATH"
...
```powershell
mkdir ~/dotfiles-test/experiment
...
```powershell
# テスト用ディレクトリ削除
Remove-Item -Recurse -Force ~/dotfiles-test
Remove-Item -Recurse -Force ~/dotfiles-test-clone
...
Rename-Item ~\.claude.bak ~\.claude

Remove-Item -Recurse -Force ~\.codex
Rename-Item ~\.codex.bak ~\.codex

Remove-Item -Recurse -Force ~\.agents
Rename-Item ~\.agents.bak ~\.agents
```
