...
- **自動化の範囲を分離**：設定ファイルは `auto-sync`、インフラ（スクリプト・hook・README）は手動管理にする。
...
`auto-sync` 対象カテゴリ（現在は `ai-agent/` のみ）のファイル保存を監視し、自動で commit / push する。
...
- 他のカテゴリ（`editor/`, `shell/`）は将来 opt-in 可能な設計。
- インフラ（`.infra/`, README）は `auto-sync` の対象外。手動で commit / push する。
...
- **2層管理**：設定ファイルはファイル監視で `auto-sync` する。インフラ（スクリプト・hook）は手動管理する。
