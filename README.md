...
- Git追跡済みのautoカテゴリ自体が消失している場合は、誤削除としてそのカテゴリを自動同期から一時的に除外する。他のautoカテゴリの同期は継続する。

### autoカテゴリの廃止

autoカテゴリを意図的に廃止するときは、ディレクトリを直接削除せず専用コマンドを使う。

```bash
bash .infra/sync.sh delete-category editor
```

このコマンドは、指定カテゴリが `SYNC_AUTO` に含まれることを確認したうえで、以下を1つのcommitとしてpushする。

- カテゴリディレクトリ全体の削除
- `.infra/conf.sh` の `SYNC_AUTO` からカテゴリを削除

カテゴリ配下は、未追跡・ignoredファイルを含めて完全に削除される。コマンド名とカテゴリ名の指定を削除確認とするため、追加の確認プロンプトは表示しない。

誤ってカテゴリディレクトリを削除した場合は、自動pushが表示する案内に従って `git restore -- <category>` で復元する。
