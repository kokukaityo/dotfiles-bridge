...
cmd_push() {
    ...
    if [ ${#missing_categories[@]} -gt 0 ]; then
        ...
        echo "  dotfiles delete-category <category>" >&2
    fi
...
cmd_delete_category() {
    if [ "$#" -ne 1 ]; then
        echo "Usage: dotfiles delete-category <category>" >&2
        return 1
    fi
...
case "${1:-help}" in
    ...
    status)    cmd_status ;;
    *)
        echo "Usage: dotfiles {pull|push|delete-category|gitignore|status}"
